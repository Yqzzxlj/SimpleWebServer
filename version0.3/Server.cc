...
void net::HttpServer::run() {
  ...
  while (true) {
    ...
    auto func = std::bind(&HttpServer::handle_request, this, std::placeholders::_1);
    thread_pool.enqueue(func, client_socket);
  }
}

void net::HttpServer::handle_request(std::shared_ptr<ClientSocket> clientSocket) {
  util::Buffer buffer;
...
void net::HttpServer::send(const http::HttpResponse& response, std::shared_ptr<ClientSocket> clientSocket) {
