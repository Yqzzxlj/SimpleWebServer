...
void net::HttpServer::run() {
  ...
  while (true) {
    std::vector<std::shared_ptr<http::HttpData>> events = Epoll::poll(serverSocket, 1024, 1000);
    for (auto req : events) {
    ...
    Epoll::timer_manager.handle_expired_event();
  }
}
...
void net::HttpServer::handle_request(std::shared_ptr<http::HttpData> http_data) {
  ...
  again:
    ...
    if (recv_data == 0) {
        std::cout << "connection closed by peer" << http_data->client_socket->fd << std::endl;
        break;
    }
    ...
    if (retcode == http::HttpRequestParser::GET_REQUEST) {
      ...
      auto it = http_data->request->headers.find(http::HttpRequest::Connection);
      if (it != http_data->request->headers.end()) {
        if (it->second == "keep-alive") {
          http_data->response->close_connection = false;
          http_data->response->headers.insert({"Keep-Alive", std::string("timeout=5")});
        } else {
          http_data->response->close_connection = true;
        }
      }
      header(http_data);
      getMime(http_data);
      ...
      send(http_data);
      if (!http_data->response->close_connection) {
        Epoll::modfd(http_data->epoll_fd, http_data->client_socket->fd, Epoll::DEFAULT_EVENTS, http_data);
...
void net::HttpServer::send(std::shared_ptr<http::HttpData> http_data) {
  ...
  close(filefd);
  std::cout << "send a file to " << http_data->client_socket->fd << std::endl;
  return;
}